...
/*
 * Computes Matrix-Vector-Product of matrix mat and vector R
...
void authTest() {
  ...
  int checksum;
  uint8_t temp2[keySize];
  uint8_t check[n/8 + (n%8 != 0)] = {0};

  for(int i=0; i<keySize; i++) {
    temp2[i] = s[i] & (v[i] ^ bv[i]);
  }

  matrixVectorProduct(&check[0], r, temp2);
  ...
  printBitString(&check[0], n/8 + (n%8 != 0));
  Serial.println();

  checksum = hammingWeight(check, n/8 + (n%8 != 0));
